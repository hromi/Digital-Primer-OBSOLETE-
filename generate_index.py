...
for label in sorted(labels):
    ...
    #label_image.write_text(0,0,label, font_filename='Fonts/'+font+'.otf',font_size='fill', max_height=70, max_width=500,color=(0,0,0))
    cue_image=Image.open(folio_dir+label)
    row=int(counter/2)
    if not counter%2:
        index_image.paste(cue_image.crop((0,0,600,600)).resize((200,150),Image.LANCZOS),(50,(int(row*150)+40)))
    else:
        index_image.paste(cue_image.crop((0,0,600,600)).resize((200,150),Image.LANCZOS),(350,(row*150)+40))
    counter+=1
